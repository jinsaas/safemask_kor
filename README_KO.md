...
[Notice] ComfyUI의 라텐트 관리 정책 변경(라텐트 봉인 및 강제 디코드)으로 인해  
직접적인 잠재값 조작이 불가능합니다. 라텐트 관련 노드는 중단되었습니다.
라텐트 합성 마스크는 파기합니다.

그로우마스크와 크롭마스크, 셀렉트 크롭의 경우 최대값 오버하면 깨지던 버그가 있었습니다.
최대값을 받은 이미지에 비례해 처리하는 식으로 바꿨습니다. 
-크롭/셀렉트 크롭은 원본보다 높은 값 넣으면 안 잘립니다.
-그로우마스크는 축소부분을 처리 안하게 바꿨고, 원본의 25배 이상은 처리하지 않습니다. 굳이 더 크게 하시려면 추가로 붙이세요.
-그로우마스크의 축소부분 동작이 필요하시면 쉬링크마스크 쓰시면 됩니다. 안정성을 위해 쪼갰습니다.
-그로우마스크 원본의 기능을 좀 더 안정적으로 쓰고싶은 분들 있을거같아서 축을 픽셀단위로 늘리는 변형마스크 놔뒀습니다.
...
#### Safe CropMask Node : 안정화는 했지만 잘라내는 위치는 추가조정 필요.
- **Inputs**:  
  - `mask`: 마스크 텐서  
  - `x`, `y`: 잘라낼 위치  
  - `width`, `height`: 잘라낼 크기  
...
#### Safe Select CropMask Node : 신규노드. 상하좌우로 직접 자를 픽셀을 지정.
- **Inputs**:  
  - `mask`: 마스크 텐서  
  - `L`, `R`, `T`, `B`: 잘라낼 위치 및 크기
- **Outputs**:  
...
#### Safe MaskComposite Node : 충돌 원인을 감소. 안정화 노드. 오류발생시 리턴하여 안정화를 유도해 재출력함.
 - **Inputs**:  
  - `mask_a`: 첫 번째 마스크  
  - `mask_b`: 두 번째 마스크  
...
#### Safe GrowMask Node : 실험노드. 안정화는 했지만 결과는 보장안됨.
- **Inputs**:  
  - `mask`: 마스크 텐서  
  - `amount`: 확장할 픽셀 수  
- **Outputs**:  
...
#### Safe ThresholdMask Node : 실험노드. 안정화는 했지만 결과는 보장안됨.
- **Inputs**:  
  - `mask`: 마스크 텐서  
  - `threshold`: 임계값 (0–1)  
- **Outputs**:  
...
#### Safe SolidMask Node : 충돌 원인을 감소. 안정화 노드. 오류발생시 리턴하여 안정화를 유도해 재출력함.
- **Inputs**:  
  - `width`: 마스크 너비  
  - `height`: 마스크 높이  
  - `value`: 채울 값 (0–1)  
...
#### Safe GrowMask Node : 안정화 노드. 마스크의 픽셀 크기를 늘림. 최대제한 픽셀을 25로 변경. 재확장시 추가연결하면 됩니다.
- **Inputs**:  
  - `mask`: 마스크 텐서  
  - `amount`: 확장할 픽셀 수  
- **Outputs**:  
...
#### Safe ShrinkMask Node : 신규노드. 마스크의 픽셀을 입력치에 비례해서 축소합니다.
- **Inputs**:  
  - `mask`: 마스크 텐서  
  - `amount`: 축소할 픽셀 수  
- **Outputs**:  
...
#### Safe TransformMask Node : 신규노드. 마스크를 입력치에 맞춰 크기를 조정. 픽셀단위로 조정합니다.
- **Inputs**:  
  - `mask`: 마스크 텐서  
  - `amount`: 조정할 가로 픽셀 수  
  - `amount`: 조정할 세로 픽셀 수  
...
#### SafeMaskChecker Node : 안정화 노드. 원본이미지의 마스크 위에 수정마스크를 덮어서 이미지 미리보기에 연결하여 사전확인 가능하게 함.(깨지던 버그 수정.)
- **Inputs**: 
...
- **Inputs**: 
  - `base_mask` : 원본 마스크(원본이미지를 마스크 컨버트로 출력후 연결)
  - `edit_mask` : 수정 마스크(마스크 에디터로 수정영역 그린 후 연결)
  - `color` : 강조 색상(red/green/blue)
- **Outputs**: 
